...
export function toCryptoTaxCurrencyTicker(currency: string): string {
    ...
    if (currency.includes('-') || currency.includes('.') || currency.includes('/')) {
...
export function parseMidgardAsset(asset: string): {blockchain: string, currency: string} {
    const elements: string[] = asset.includes('.') ? asset.split('.') : asset.split('/');

    if (elements.length !== 2) {
        throw new Error(`Unable to determine blockchain and currency from "${asset}"`);
    }
...
export function isThorchainVault(address: string): boolean {
    const vaults = [
        'bc1qvk5jr06g4kadhep03lpz70kp5ya0j0rjddefcg'
    ];
